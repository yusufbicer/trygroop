          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="text-xl">
                      {order.title}
                      {order.product_name && (
                        <span className="ml-2 text-white/70">({order.product_name})</span>
                      )}
                    </CardTitle>
                    <CardDescription>
                  ...
                  <Badge 
                    className={`
                      ${order.status === 'completed' ? 'bg-green-500/20 text-green-500 hover:bg-green-500/30' : 
                        order.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500 hover:bg-yellow-500/30' : 
                        order.status === 'processing' ? 'bg-blue-500/20 text-blue-500 hover:bg-blue-500/30' : 
                        order.status === 'shipped' ? 'bg-purple-500/20 text-purple-500 hover:bg-purple-500/30' : 
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  {order.amount !== undefined && (
                    <div>
                      <h3 className="text-sm font-medium text-white/70 mb-1">Amount</h3>
                      <p className="text-white">${parseFloat(order.amount).toFixed(2)}</p>
                    </div>
                  )}
                  
                  {order.total_volume !== null && (
                    <div>
                      <h3 className="text-sm font-medium text-white/70 mb-1">Total Volume</h3>
                      <p className="text-white">{order.total_volume} m³</p>
                    </div>
                  )}
            ...
            <Card className="glass">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    <h3 className="text-sm font-medium text-white/70 mb-1">Customer ID</h3>
                    <p className="text-white">{order.user_id}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-white/70 mb-1">Name</h3>
                    <p className="text-white">
                      {order.profile?.first_name || ''} {order.profile?.last_name || ''}
                      {!order.profile?.first_name && !order.profile?.last_name && 'N/A'}
                    </p>
                  </div>
                </div>
              </CardContent>
